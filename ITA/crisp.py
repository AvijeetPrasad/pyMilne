...
class crisp:
    def __init__(self, w0):
        ...
        self.calp = np.cos(np.sqrt(np.asarray((0.04691007703067, 0.23076534494716, 0.50,
                           0.76923465505284, 0.95308992296933), dtype='float64')*(0.5/165.)**2)) * self.pi2
        self.wng = np.asarray((0.11846344252809, 0.23931433524968, 0.28444444444444,
                              0.23931433524968, 0.11846344252809), dtype='float64')
    ...
    def read_reflectivity(self, w0):
        ...
        if ((w0 > ma) or (w0 < mi)):
            print(
                inam + "Warning, w0={0} is outside the valid range [{1}, {2}], taking value at the edge".format(
                    w0, int(mi), int(ma)))
