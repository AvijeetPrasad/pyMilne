...
# comp_flags=['-Ofast', '-flto','-g0','-fstrict-aliasing','-march=native',
# '-mtune=native','-std=c++20','-fPIC','-fopenmp', '-I./src',
# "-DNPY_NO_DEPRECATED_API",'-mprefer-vector-width=256', '-DNDEBUG',
#  '-pedantic', '-Wall', '-I/mn/stornext/d9/data/avijeetp/envs/pymilne/include/eigen3']

comp_flags = ['-Ofast', '-g0', '-fstrict-aliasing', '-march=native',
              '-mtune=native', '-std=c++20', '-fPIC', '-fopenmp', '-I./src',
              "-DNPY_NO_DEPRECATED_API", '-mprefer-vector-width=256', '-DNDEBUG',
              '-pedantic', '-Wall', '-I/mn/stornext/d9/data/avijeetp/envs/pymilne/include/eigen3']

# comp_flags=['-Ofast','-g0','-fstrict-aliasing','-march=native','-mtune=native',
# '-std=c++20','-fPIC','-fopenmp', '-I./src', "-DNPY_NO_DEPRECATED_API",'-mprefer-vector-width=256',
# '-DNDEBUG', '-pedantic', '-Wall','-I/mn/stornext/d9/data/carlosjd/mamba_envs/kdb/include/eigen3']

# Optimization flags for development

# comp_flags=['-Og', '-g3','-fstrict-aliasing','-march=native','-mtune=native','-std=c++20','-fPIC','-fopenmp',
#  '-I./src', "-DNPY_NO_DEPRECATED_API", '-pedantic', '-Wall']
