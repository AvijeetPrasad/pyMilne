...
if __name__ == "__main__":
    ...
    me = MilneEddington.MilneEddington(
        regions, lines, nthreads=2, precision=dtype)
    ...
    #
    sig = np.zeros((4, me.get_wavelength_array().size),
    ...
    # invert
    #
    model_out, syn_out, chi2 = me.invert(
        guessed_initial, syn, sig, nRandom=5, nIter=20, chi2_thres=1.0, verbose=False)
    ...
    for ii in range(9):
        print('{0:13.5f} -----> {1:13.5f} +/- {2:8.5f}'.format(
            m_in[ii], model_out[0, 0, ii], errors[0, 0, ii]))
    ...
    for ii in range(4):
        if (ii == 0):
            ...
            ax1[ii].plot(wav, syn_out[0, 0, ii], '-',
                         color='orangered', label='Fit')
    ...
    # f.show()
    # Save the plot as an image file instead of showing it interactively
    plt.savefig('synthetic_spectra.png')
