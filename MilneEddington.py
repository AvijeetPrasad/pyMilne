...
class MilneEddington:
    ...
    def _initLine(self, label, anomalous, dw, precision):
        ...
        if (precision == 'float64'):
            ...
            if (label == 6301):
                return pyMilne.pyLines(j1=2.0, j2=2.0, g1=1.84, g2=1.50, cw=6301.4995, gf=10.**-0.718,
                                       anomalous=anomalous, dw=dw)
            elif (label == 6302):
                return pyMilne.pyLines(j1=1.0, j2=0.0, g1=2.49, g2=0.00, cw=6302.4931, gf=10.**-1.160,
                                       anomalous=anomalous, dw=dw)
            elif (label == 6173):
                return pyMilne.pyLines(j1=1.0, j2=0.0, g1=2.50, g2=0.00, cw=6173.3340, gf=10.**-2.880,
                                       anomalous=anomalous, dw=dw)
            else:
                print(
                    "pyLines::setLine: Error line with label {0 } is not implented".format(label))
                return pyMilne.pyLines()
        else:
            if (label == 6301):
                return pyMilne.pyLinesf(j1=2.0, j2=2.0, g1=1.84, g2=1.50, cw=6301.4995, gf=10.**-0.718,
                                        anomalous=anomalous, dw=dw)
            elif (label == 6302):
                return pyMilne.pyLinesf(j1=1.0, j2=0.0, g1=2.49, g2=0.00, cw=6302.4931, gf=10.**-1.160,
                                        anomalous=anomalous, dw=dw)
            elif (label == 6173):
                return pyMilne.pyLinesf(j1=1.0, j2=0.0, g1=2.50, g2=0.00, cw=6173.3340, gf=10.**-2.880,
                                        anomalous=anomalous, dw=dw)
            else:
                print(
                    "pyLines::setLine: Error line with label {0 } is not implented".format(label))
    ...
    def __init__(self, regions, lines, anomalous=True, dw_lines=20,  nthreads=1, precision='float64'):
        ...
        Arguments:
             ...
             dw_lines: spectral window +/- dw from line center to compute the line profile.
                       Outside that window the profile won't be calculated.
        ...
        for ii in range(len(regions)):
            if (len(regions[ii]) != 2):
                print("MilneEddington::__init__: ERROR, region {0} has {1} elements, should have 2!".format(
                    ii, len(regions[ii])))
                error = True
        ...
        if (precision == 'float32'):
            self.Me = pyMilne.pyMilne_float(
                regions, pyLines, nthreads=nthreads, anomalous=anomalous)
        else:
            self.Me = pyMilne.pyMilne(
                regions, pyLines, nthreads=nthreads, anomalous=anomalous)
    ...
    def synthesize(self, model, mu=1.0):
        ...
        The model parameters are: [|B| [G], inc [rad], azi [rad], vlos [km/s], vDop [A], eta_l, damp, S0, S1]
        ...
        if (ndim == 1):
            model1 = np.ascontiguousarray(
                model.reshape((1, 1, model.size)), dtype=dtype)
        elif (ndim == 3):
            model1 = model
        else:
            print(
                "MilneEddington::synthesize: ERROR, the input model must have 1 or 3 dimensions")
            return None

        if (model1.shape[2] != 9):
            print("MilneEddington::synthesize: ERROR, input model has npar={0}, should be 9".format(
                model1.shape[2]))
            return None
    ...
    def synthesize_rf(self, model, mu=1.0):
        ...
        if (ndim == 1):
            ...
            print(
                "MilneEddington::synthesize_rf: ERROR, the input model must have 1 or 3 dimensions")
            return None

        if (model1.shape[2] != 9):
            print("MilneEddington::synthesize_rf: ERROR, input model has npar={0}, should be 9".format(
                model1.shape[2]))
            return None
    ...
    def invert(self, model, obs, sig=1.e-3, mu=1.0, nRandom=3, nIter=20, chi2_thres=1.0, verbose=False):
        ...
        Arguments:
              model: 1D [9] or 3D array [ny,nx,9] with the parameters of the model
                obs: 2D [4,nwav] or 4D array [ny,nx,4,nwav] with the observed profiles.
                Should be normalized to the mean continuum.
                sig: scalar or 2D array [4,nwav] with the noise estimate

                 mu:    heliocentric angle for the synthesis
            nRandom: if larger than 1, the input model parameters will be randomized and more inversion
                     will be performed
        ...
        if (ndim == 2):
            obs1 = np.ascontiguousarray(model.reshape(
                (1, 1, obs.shape[0], obs.shape[1])), dtype=dtype)
        elif (ndim == 4):
            obs1 = obs
        else:
            print(
                "MilneEddington::invert: ERROR, the input observations must have 2 or 4 dimensions")
            return None, None, None

        wav = self.Me.get_wavelength_array()
        nwav = wav.size
        if (obs1.shape[3] != nwav):
            print("MilneEddington::invert: ERROR, input observations has nwav={0}, should be nwav={1}".format(
                obs1.shape[3], nwav))
            return None, None, None
        ...
        if isinstance(sig, np.ndarray):
            if (sig.shape[1] != nwav):
                print("MilneEddington::invert: sigma array has nwav={0}, but it should be {1}".format(
                    sig.shape[1], nwav))
                return None, None, None
        ...
        # Call C++ module
        #
        return self.Me.invert(model1, obs1, sig1, mu=mu, nRandom=nRandom, nIter=nIter, chi2_thres=chi2_thres,
                              verbose=verbose)
    ...
    def repeat_model(self, m_in, ny, nx, nt=None):
        ...
        if (nPar != 9):
            print(
                "MilneEddington::repeat_model: Error, input model must have 9 elements!")
            return None
    ...
    def invert_spatially_regularized(self, model, obs, sig=1.e-3, mu=1.0, nIter=20, chi2_thres=1.0,
        ...
        Arguments:
              model: 1D [9], 3D array [ny,nx,9] or 4D array [nt,ny,nx,9] with the parameters of the model
                obs: 2D [4,nwav], 4D array [ny,nx,4,nwav] or 5D array [nt,ny,nx,4,nw] with the observed profiles.
                     Should be normalized to the mean continuum.
                sig: scalar or 2D array [4,nwav] with the noise estimate
                 mu:    heliocentric angle for the synthesis
              nIter: maximum number of Levenberg Marquardt iterations per inversion
       ...
       x alpha_time: global time regularization weight that multiplies the value of "alphas_time" (default = 1).
        ...
        alphas_time: the relative scaling of regularization weights for each parameter in time (default = 1).
             ...
             method: Numerical method to solve the sparse system:
                     0) Conjugate Gradient, 1) BiCGStab, 2) SparseLU (default 1)
      delay_bracket: Delay optimal lambda bracketing for this number of iterations.
                     Avoids taking too large steps in the initial iterations.
        The model parameters are: [|B| [G], inc [rad], azi [rad], vlos [km/s], vDop [A], eta_l, damp, S0, S1]
        ...
        if (ndim == 1):
            model1 = np.ascontiguousarray(
                model.reshape((1, 1, 1, model.size)), dtype=dtype)
        elif (ndim == 3):
            ...
            print("MilneEddington::invert_spatially_regularized_float:\
                   ERROR, the input model must have 1, 3 or 4 dimensions")
            return None, None, None

        if (model1.shape[3] != 9):
            print("MilneEddington::invert_spatially_regularized_float:\
                   ERROR, input model has npar={0}, should be 9".format(
                model1.shape[2]))
            return None, None, None
        ...
        if (ndim == 2):
            ...
            print("MilneEddington::invert_spatially_regularized_float:\
                   ERROR, the input observations must have 2, 4 or 5 dimensions")
            return None, None, None
        ...
        if (obs1.shape[4] != nwav):
            print("MilneEddington::invert_spatially_regularized_float:\
                   ERROR, input observations has nwav={0}, should be nwav={1}".format(
                obs1.shape[3], nwav))
            return None, None, None
        ...
        if isinstance(sig, np.ndarray):
            if (sig.shape[1] != nwav):
                print("MilneEddington::invert_spatially_regularized_float:\
                       sigma array has nwav={0}, but it should be {1}".format(
                    sig.shape[1], nwav))
                return None, None, None
        ...
        # Call C++ module
        #
        return self.Me.invert_spatially_regularized(model1, obs1, sig1, alphas_in, alphas_time_in, betas_in, mu=mu,
                                                    nIter=nIter, chi2_thres=chi2_thres,  method=method,
                                                    delay_bracket=delay_bracket, iLam=init_lambda)
    ...
    def invert_spatially_coupled(self, model, spat_regions, mu=1.0, nIter=20,
       ...
       spat_regions: list of lists [[obs1,sigma1,psf1], obs2,sigma2,psf2] with the observations and their PSF
         ...
         chi2_thres: stop inversion if Chi2 <= chi2_thres
             ...
             alphas: the relative scaling of regularization weights for each parameter (default = 1).
      delay_bracket: Delay optimal lambda bracketing for this number of iterations.
                     Avoids taking too large steps in the initial iterations. (TODO)
        The model parameters are: [|B| [G], inc [rad], azi [rad], vlos [km/s], vDop [A], eta_l, damp, S0, S1]
        ...
        if (ndim == 1):
            ...
            print("MilneEddington::invert_spatially_regularized_float:\
                   ERROR, the input model must have 1 or 3 dimensions")
            return None, None, None
        ...
        return self.Me.invert_Spatially_Coupled(model1, spat_regions, alphas_in,  mu=mu, nIter=nIter,
                                                chi2_thres=chi2_thres,  method=0, delay_bracket=delay_bracket,
                                                iLam=init_lambda)
